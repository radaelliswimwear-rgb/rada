...
- **Completar el Panel Administrativo** — el Sprint 14 construyó productos, categorías (rename), inventario, pedidos, usuarios y dashboard, con buscador y paginación; falta Cloudinary para imágenes, CRUD completo de categorías (exige rutas dinámicas), buscador/paginación server-side si el volumen crece mucho más, y una entrada visible en el Navbar para administradores — ver [ADMIN_PANEL.md](./ADMIN_PANEL.md).
