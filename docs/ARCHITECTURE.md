...
- ✅ **Pagos** (`lib/payments/`) — Postgres/Prisma desde el Sprint 12 (antes localStorage, Sprint 11). `providers/stripe-gateway.ts` sigue simulado (sin red real); `providers/wompi-gateway.ts` habla de verdad con la API de Wompi desde el Sprint 16 (tokenización + transacciones + webhook, ver [ADMIN_PANEL.md](./ADMIN_PANEL.md) y [sprints/SPRINT-16.md](./sprints/SPRINT-16.md)) — sin credenciales configuradas en este entorno, no verificado en vivo.
...
- ✅ **Blog, Newsletter y Cupones** (`lib/blog/`, `lib/newsletter/`, `lib/coupons/`, `lib/admin/{blog,newsletter,coupons}-actions.ts`, Sprint 17) — mismo patrón Server Actions + Repository, con el mismo matiz lectura-pública-resiliente vs. escritura-administrativa-explícita que el resto (`lib/blog/blog-actions.ts` degrada a `[]`/`null`; `lib/admin/blog-actions.ts` devuelve `AdminActionResult`). Sin servicios externos: el blog usa un conversor Markdown propio (`lib/blog/markdown.ts`, sin librería nueva) y la newsletter persiste suscriptores/campañas en Postgres sin enviar ningún email real (no hay proveedor configurado). Los cupones (`Order.couponCode`/`discountValue`, opcionales) se validan en el checkout y solo consumen un uso (`Coupon.usedCount`) después de que el pedido se crea con éxito, nunca al solo probar el código.
...
`lib/payments/providers/stripe-gateway.ts` no llama a ningún SDK ni API externa: `confirmPayment` espera una latencia simulada (`simulateLatency`) y decide éxito/fallo según el número de tarjeta, replicando las tarjetas de prueba reales de Stripe (`4242...` éxito, `4000...0002` rechazo) para que la demo se sienta realista. Ningún dato de tarjeta se valida contra un emisor real ni se envía a ningún lado. Al conectar Stripe de verdad, ese archivo se reemplaza por el SDK real (Payment Intents API) sin tocar `payments-repository.ts` ni `components/checkout/payment-form.tsx` — mismo contrato `PaymentGateway`.

`lib/payments/providers/wompi-gateway.ts` sí es real desde el Sprint 16: tokeniza la tarjeta y crea la transacción contra la API de Wompi (sandbox por defecto, `WOMPI_BASE_URL`), con una firma de integridad (`SHA-256` de referencia+monto+moneda+secreto) exigida por Wompi. `app/api/webhooks/wompi/route.ts` recibe las actualizaciones asincrónicas de estado, verifica su propia firma (`WOMPI_EVENTS_SECRET`) y actualiza el `Payment` — si ese pago ya estaba vinculado a un pedido y termina fallando, el pedido se cancela automáticamente (`lib/payments/payments-actions.ts#applyWompiWebhookUpdateAction`). Ver limitaciones (sin credenciales verificadas en este entorno, checkout de invitado sin fila `User "guest"`) en [sprints/SPRINT-16.md](./sprints/SPRINT-16.md).

La cancelación desde la UI (botón "Cancelar pago" durante el procesamiento) sigue siendo un aborto puramente local para ambos proveedores: no cancela nada del lado del proveedor real, solo descarta el resultado y marca el intento como `cancelled` en el registro local.

### SEO (Sprint 17)

`lib/seo/site.ts` centraliza las constantes de marca (nombre, descripción, URL, logo) y `lib/seo/json-ld.tsx` expone un componente `<JsonLd data={...} />` para no repetir `<script type="application/ld+json">` a mano — mismo patrón que ya usaba `app/product/[handle]/page.tsx` (track Shopify dormido), ahora reutilizable en `app/layout.tsx` (Organization + WebSite), `app/producto/[slug]/page.tsx` (Product) y `app/blog/[slug]/page.tsx` (Article).

`app/sitemap.ts` dependía de `lib/shopify` (`validateEnvironmentVariables()`, que exige credenciales de Shopify no configuradas en este proyecto) y devolvía 500 siempre — se reescribió para leer de `catalogRepository`/`blogRepository`, sin tocar el resto del catálogo. `app/robots.ts` agrega `disallow` para las rutas privadas/transaccionales.
